...
export const OLLAMA_MODEL_STORAGE_KEY: Record<ModelId, string> = {
...
export const OLLAMA_MODEL_DEFAULT_TAG: Record<ModelId, string> = {
  E2B: 'gemma4:e2b',
  E4B: 'gemma4:e4b',
  '27B': 'gemma4:27b',
};

const TIERS: ModelId[] = ['E2B', 'E4B', '27B'];

/** Raw value in localStorage (may be empty). SSR-safe. */
export function readOllamaModelOverrideRaw(tier: ModelId): string {
...
export function getOllamaModelTagForTier(tier: ModelId): string {
  const trimmed = readOllamaModelOverrideRaw(tier).trim();
  return trimmed.length > 0 ? trimmed : OLLAMA_MODEL_DEFAULT_TAG[tier];
}

export function writeOllamaModelOverride(tier: ModelId, value: string): void {
  if (typeof window === 'undefined') return;
